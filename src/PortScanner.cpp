...
void PortScanner::create_ip_header(iphdr *iph,
...
void PortScanner::create_tcp_header(tcphdr *tcp_header,
                                                             iphdr *ip_header,
                                                              ...
                                                              u_int8_t flags)
{
    ...
    tcp_header->check = generate_tcp_checksum(ip_header,
                                              tcp_header,
                                              (ip_header->tot_len-(ip_header->ihl*4)));
}

void PortScanner::create_udp_header(udphdr* udp_header,
                                                               iphdr *ip_header,
...
std::string PortScanner::get_source_ip(sockaddr_in &source)
{
...
void PortScanner::tcp_scan(const char * dest,
                                             unsigned int d_port,
                                             std::string s_type)
{
    ...
    memcpy(&destination,
                (sockaddr_in *) list_pointer->ai_addr,
                list_pointer->ai_addrlen);
    ...
    if (setsockopt (write_sock,
                            IPPROTO_IP,
                            IP_HDRINCL,
                            temp_val,
                            sizeof (option)) < 0)
    {
        logdbg("Error setting IP_HDRINCL");
        exit(0);
    }
    
    if(setsockopt(read_sock,
                           SOL_SOCKET,
                           SO_RCVTIMEO,
                           (timeval *)&time_val,
                           sizeof(timeval)) != 0)
    ...
    create_ip_header(iph,source.sin_addr.s_addr ,
                                destination.sin_addr.s_addr,
                                sizeof(iphdr)+ sizeof(tcphdr), IPPROTO_TCP);
    ...
    //time to send
    do{
        sent_bytes = sendto(write_sock, final_header,
                                        sizeof(tcphdr) + sizeof(iphdr),
                                        0,
   ...
   // iphdr* read_iphdr = (iphdr*) read_buffer;
    ...
    if ( read_iphdr->protocol==IPPROTO_TCP )
    {
        ...
        if ( read_iphdr->protocol==IPPROTO_ICMP)
        {
            ...
            if(read_icmphdr->type == 3
               && (read_icmphdr->code == 1 || read_icmphdr->code == 2 ||        read_icmphdr->code == 3    || read_icmphdr->code == 9 || read_icmphdr->code == 10  || read_icmphdr->code == 13))
                status_report->status= "FILTERED";
            else
                status_report->status= "UNFILTERED";
            read_icmphdr = NULL;
...
void PortScanner::udp_scan(const char* dest , unsigned int d_port)
{
    ...
    dns = (struct dns_header *)&dns_buffer[sizeof( struct iphdr )
    ...
    qname = (unsigned char*)&dns_buffer[sizeof(struct dns_header)
                                        + sizeof(struct iphdr)
                                        + sizeof(struct udphdr)];
    dns_format_convert(qname , dns_host);
    qinfo = (struct question*) &dns_buffer[sizeof(struct dns_header)
   ...
   /*-------------------------*/
    ...
    char final_header[4096];
    //========================
    write_sock = socket(AF_INET, SOCK_RAW, IPPROTO_UDP);
    if(write_sock < 0 )
    {
        logerr() << "\n\t Socket creation failed";
    }
    read_sock = socket(AF_INET,SOCK_RAW, IPPROTO_ICMP);
    if(read_sock  < 0)
    {
        logerr() << "n\t ICMP socket creation failed";
    }
    int option = 1;
    const int *temp_val = &option;
    ...
    if (setsockopt (write_sock,
                             ...
                             sizeof (option))    < 0)
    {
        logerr() << "Error setting IP_HDRINCL";
    }
    ...
    do{
        if( d_port == 53 )
            sent_bytes = sendto(write_sock, dns_buffer,
                                             iph->tot_len,
                                             0,
                                             (sockaddr *)&destination,
                                             sizeof(destination));
        else
            sent_bytes = sendto(write_sock,
                                             final_header,
                                             sizeof(udphdr) + sizeof(iphdr),
                                             0,
                                             (sockaddr *)&destination,
                                             sizeof(destination));
        time(&timer);
       ...
       //ssize_t data_read = 0;
        ...
        sleep(2);
        ssize_t received_bytes = recv(read_sock,
                                                         read_buffer ,
                                                         sizeof(read_buffer),
                                                         0);
        if(received_bytes < 0 )
    ...
    if ( read_iphdr->protocol==IPPROTO_ICMP )
    {
        ...
        if((read_icmphdr->type  == ICMP_DEST_UNREACH)
           && (read_icmphdr->code == ICMP_PORT_UNREACH))
        {
            status_report->protocol = IPPROTO_UDP;
            status_report->status = "CLOSED";
        }
...
void PortScanner::service_scan(const char* dest_host, const char* service)
{
    ...
    {
        logerr() <<  "getaddrinfo() failed " <<  gai_strerror(r_value) ;
    }
    ...
    if ( it == NULL)
    {
        logerr()   <<  " Error getting address ";
    }
    freeaddrinfo(servinfo);
    /*** HTTP Check ****/
    if ( std::strcmp(service, "80") == 0)
    {
        ...
        bytes_recvd = recv(ssock,recv_buffer, 2000,0);
        if( bytes_recvd == -1 ) //We actually just want some lines
        {
            logerr() << "\n\t\t recv() failed \n";
        }
        ...
        {
            logdbg("Cannot determine HTTP version. BAD request perhaps ");
        }
        else
    ...
    if ( std::strcmp(service, "43") == 0)
    {
        ...
        bytes_recvd = recv(ssock,recv_buffer, 5000,0);
        if( bytes_recvd == -1 ) //We actually just want some lines
        {
            logerr() << "\n\t\t recv() failed \n";
        }
    ...
    if ( std::strcmp(service, "143") == 0)
    {
        ...
        char *resp = strstr(recv_buffer, "Dovecot");
        if ( resp == NULL)
            logdbg(" IMAP service not running");
    ...
    if ( std::strcmp(service, "22") == 0)
    {
        ...
        if( ssh_version != "2.0")
        {
            logdbg( "Cannot determine SSH version" );
        }
        else
 ...
 void PortScanner::begin_scan(int argc, char ** argv)
 {
    ArgumentParsing a;
    a.parsing_args(argc,argv);
    std::string scantype;
    int port;
    std::vector<Report>::iterator it;
    ...
    for(int i=0;i < a.ipAddress.size(); i++)
    {
        for(int j=0; j < a.portNo.size(); j++)
        {
            for(int k=0; k < a.scanlist.size(); k++)
            {
        ...
        for(int i=0; i < a.ipAddress.size(); i++)
        {
            for(int j=0; j < a.portNo.size(); j++)
            {
                for(int k=0; k < a.scanlist.size(); k++)
                {
                    ...
                    if ( (port >=0 && port <= 1024)
                      && port != 22 && port != 24 && port != 43 && port != 80
                      && port != 110 && port != 143 )
    ...
    logdbg("Scanning....");
    logdbg("Scan took : " <<  (time_end - time_start));
    for(int i=0;i  <  a.ipAddress.size();   i++)
    {
        logdbg("IP Address : " << a.ipAddress[i] );
        logdbg( "OPEN Ports :" );
        logdbg( std::setw(11) << "Port" << std::setw(30)
               << "Service Name" << std::setw(35) << "Results"
               << std::setw(37) << "Conclusion");
        //int count = 0;
        
        r1.print_output(0,a.ipAddress[i]);              
        logdbg( "Closed/Filtered/Unfiltered Ports:");
        logdbg( std::setw(10) <<  "Port " <<  std::setw(37)
               << "Service Name " << std::setw(30) << "Results"
               << std::setw(34) << "Conclusion");
